...
public interface ThreadRepo extends JpaRepository<Thread, Long> {
    ...
    @Query(value = "SELECT new com.eg.yafi.projection.ReadThread(t.id AS id, topic.id AS topicId, topic.name AS topicName, t.content AS content," +
    ...
    @Query(value = "SELECT new com.eg.yafi.projection.ReadThreadExtended(t.id AS id, topic.id AS topicId, topic.name AS topicName," +
    ...
    @Query(value = "SELECT new com.eg.yafi.projection.ReadThread(t.id AS id, t.topic.id AS topicId, topic.name AS topicName, t.content AS content," +
                   "    appUser.username AS username, t.likeCount AS likeCount)" +
                   "    FROM Thread t" +
                   "    INNER JOIN Topic topic ON t.topic.id = topic.id" +
                   "    INNER JOIN AppUser appUser ON t.appUser.id = appUser.id" +
